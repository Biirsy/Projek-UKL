...
<body>
    ...
    <form action="function-hitungkal.php" method="POST">
        <div class="choice">
            <?php
            $mysqli = NEW mysqli ('localhost','root','','mulai_sehat');

            $resultSet = $mysqli->query("SELECT * FROM hitung_kalori");
            ?>
            ...
            while($rows = $resultSet->fetch_assoc()){
                ...
                $id_kalori1 = $rows['id_makanan'];
                echo "<option value='$id_kalori1'>$namaFood</option>";
            }
            ?>
            </select>
        </div>
        <div class="choice">
            ...
            while($rows = $resultSet->fetch_assoc()){
                $namaFood = $rows['nama_makanan'];
                $id_kalori2 = $rows['id_makanan'];
                echo "<option value='$id_kalori2'>$namaFood</option>";
            }
            ?>
            </select>
        </div>
        <div class="choice">
            ...
            while($rows = $resultSet->fetch_assoc()){
                $namaFood = $rows['nama_makanan'];
                $id_kalori3 = $rows['id_makanan'];
                echo "<option value='$id_kalori3'>$namaFood</option>";
            }
            ?>
            </select>
        </div>
        <div class="choice">
            ...
            while($rows = $resultSet->fetch_assoc()){
                $namaFood = $rows['nama_makanan'];
                $id_kalori4 = $rows['id_makanan'];
                echo "<option value='$id_kalori4'>$namaFood</option>";
            }
            ?>
            </select>
        </div>
        <input type="submit" name="submit" value="Hitung Kalori" class="mulaihitung">
    </form>
    <div class="footer">
